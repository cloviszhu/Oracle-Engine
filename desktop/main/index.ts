...
async function registerServices() {
  ...
  const run = async (command: string, args: string[]) => execFileAsync(command, args, { windowsHide: true, timeout: 120_000, maxBuffer: 4 * 1024 * 1024 });
  const compose = new SerenityComposeController(run, composeFile);
  ...
  const settingsService = new LauncherSettingsService({
    ...
    revokeActorSessions: async (actorId) => {
      const redis = createRedisConnection('redis://127.0.0.1:36379/0');
      try {
        await Promise.race([
  ...
  const backup = new BackupService({
    ...
    dumpDatabase: (path) => streamDatabaseDump(composeFile, path),
  });
  const diagnostics = async () => {
    ...
    return new DiagnosticsService({
      ...
      collect: async () => {
        const containers = await run('docker', ['compose', '-p', 'serenity-local', '-f', composeFile, 'ps', '--format', 'json']).then(({ stdout }) => stdout).catch(() => 'Docker 不可用');
        return { health, ports: [3000, 33060, 36379], containers, logs: ['诊断由 Electron main 脱敏导出'] };
      },
    }).export();
  };
  ...
  const orchestrator = new ServiceOrchestrator({
    prepareDirectories: async () => { runtimeSnapshot = await settingsService.getRuntimeSnapshot(); },
    reservePorts: async () => undefined,
    startInfrastructure: async () => { health = startingHealth(); await compose.startInfrastructure(); },
    waitForInfrastructure: () => waitForInfrastructure(run, composeFile),
    ...
    openWorkspace: async () => { await shell.openExternal('http://127.0.0.1:3000'); },
  ...
  registerLauncherHandlers(ipcMain, {
    ...
    runEnvironmentChecks: () => runEnvironmentChecks(createSystemEnvironmentProbes(dataDirectory, [3000, 33060, 36379])),
    ...
    startSerenity: async () => {
      try {
        ...
        health = { overall: 'healthy', workspaceUrl: 'http://127.0.0.1:3000', services: serviceNames.map((name) => ({ name, status: 'healthy' })) };
        return health;
      } catch (error) {
    ...
    stopSerenity,
    getHealth: async () => refreshHealth(orchestrator.getState(), run, composeFile, supervisor),
...
async function refreshHealth(
  ...
  const [apiHealthy, workerResponsive, workerHeartbeat] = await Promise.all([
    fetch('http://127.0.0.1:3000/api/health', { signal: AbortSignal.timeout(3_000) }).then((response) => response.ok).catch(() => false),
  ...
  health = { overall: services.every((service) => service.status === 'healthy') ? 'healthy' : 'degraded', workspaceUrl: 'http://127.0.0.1:3000', services };
  return health;
}
...
async function streamDatabaseDump(composeFile: string, destination: string): Promise<void> {
  const child = spawn('docker', ['compose', '-p', 'serenity-local', '-f', composeFile, 'exec', '-T', 'db', 'mysqldump', '-uroot', '--single-transaction', 'serenity'], { windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
  let stderr = '';
