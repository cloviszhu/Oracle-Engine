...
export function App({ launcher = window.serenityLauncher }: { launcher?: SerenityLauncherApi }) {
  ...
  return <SetupWizard launcher={launcher} existing={status.configured ? status : undefined} onSaved={(next) => { setStatus(next); setSetupStarted(false); }} />;
}
...
function SetupWizard({ launcher, existing, onSaved }: { launcher: SerenityLauncherApi; existing?: SetupStatus; onSaved(status: SetupStatus): void }) {
  const [step, setStep] = useState(0);
  const [checks, setChecks] = useState<EnvironmentCheckResult[]>([]);
  const [checking, setChecking] = useState(false);
  const [accounts, setAccounts] = useState(existing?.editable?.familyAccounts.map((account) => ({ ...account, password: '' })) ?? [
    { actorId: 'father', username: '', password: '' }, { actorId: 'requester', username: '', password: '' },
  ]);
  const [aiEnabled, setAIEnabled] = useState(existing?.editable?.ai?.enabled ?? false);
  const [ai, setAI] = useState<AISettingsInput>(existing?.editable?.ai ? { ...defaultAI, ...existing.editable.ai, apiKey: '' } : defaultAI);
  const [aiProbed, setAIProbed] = useState(Boolean(existing?.editable?.ai?.enabled && existing.ai?.configured));
  const [x, setX] = useState({ enabled: existing?.editable?.x.enabled ?? false, token: '', policyConfirmed: existing?.editable?.x.policyConfirmed ?? false });
  const [feishu, setFeishu] = useState({ enabled: existing?.editable?.feishu.enabled ?? false, webhookUrl: '', signingSecret: '' });
  ...
  const updateAI = (patch: Partial<AISettingsInput>) => { setAI((current) => ({ ...current, ...patch })); setAIProbed(false); };
  ...
  return <main className="wizard-shell">
    ...
    <section className="wizard-stage" aria-live="polite">
      ...
      {step === 1 && <><SectionTitle kicker="STEP 02" title="两位家庭成员" copy="密码只在此刻用于生成随机盐摘要，不会回显；修改设置时留空表示保持原密码。" /><div className="account-grid">{accounts.map((account, index) => <fieldset key={account.actorId}><legend>账号 {index + 1}</legend><label>账号 {index + 1} 用户名<input value={account.username} onChange={(event) => updateAccount(index, 'username', event.target.value)} autoComplete="username" /></label><label>账号 {index + 1} 密码<input type="password" value={account.password} onChange={(event) => updateAccount(index, 'password', event.target.value)} autoComplete="new-password" /></label></fieldset>)}</div><Footer next={next} disabled={accounts.some((account) => !account.username || (!existing && account.password.length < 12) || Boolean(account.password && account.password.length < 12)) || accounts[0]?.username === accounts[1]?.username} /></>}
      {step === 2 && <><SectionTitle kicker="STEP 03" title="AI 分析" copy="可以先跳过。启用时必须通过完整能力验证，不会自动换模型。" /><Toggle label="启用 AI 分析" checked={aiEnabled} onChange={(enabled) => { setAIEnabled(enabled); setAIProbed(Boolean(enabled && existing?.ai?.configured)); }} />{aiEnabled && <div className="form-grid"><label>服务预设<select value={ai.providerPreset} onChange={(e) => updateAI({ providerPreset: e.target.value as AISettingsInput['providerPreset'], baseUrl: '', ...(e.target.value === 'openai' ? { model: 'gpt-5.6-terra', protocol: 'responses' } : {}) })}><option value="openai">OpenAI 官方</option><option value="custom">自定义兼容服务</option></select></label><label>协议<select value={ai.protocol} onChange={(e) => updateAI({ protocol: e.target.value as AISettingsInput['protocol'] })}><option value="responses">Responses</option><option value="chat_completions">Chat Completions</option></select></label><label className="wide">HTTPS 服务地址（官方预设可留空）<input value={ai.baseUrl} placeholder={ai.providerPreset === 'openai' ? 'https://api.openai.com' : 'https://models.example.com'} onChange={(e) => updateAI({ baseUrl: e.target.value })} /></label><label>模型<input value={ai.model} onChange={(e) => updateAI({ model: e.target.value })} /></label><label>API Key<input type="password" value={ai.apiKey} placeholder={existing?.ai?.configured ? '留空表示使用已保存密钥' : ''} onChange={(e) => updateAI({ apiKey: e.target.value })} /></label><NumberField label="输入价格（分/百万）" value={ai.inputCostPerMillionCents} onChange={(value) => updateAI({ inputCostPerMillionCents: value })} /><NumberField label="输出价格（分/百万）" value={ai.outputCostPerMillionCents} onChange={(value) => updateAI({ outputCostPerMillionCents: value })} /><button className="secondary wide" onClick={() => void probe()} disabled={(!ai.apiKey && !existing?.ai?.configured) || !ai.model || (ai.providerPreset === 'custom' && !ai.baseUrl)}>测试 AI 连接</button>{message && <p className={aiProbed ? 'success-note' : 'muted'}>{message}</p>}{message && !aiProbed && <button className="secondary wide" onClick={() => { setAIEnabled(false); next(); }}>保存配置但保持 AI 未启用</button>}</div>}<Footer next={next} nextLabel={aiEnabled ? '下一步' : '跳过 AI'} disabled={aiEnabled && !aiProbed} /></>}
      {step === 3 && <><SectionTitle kicker="STEP 04" title="X 数据源" copy="默认关闭；未启用时不会发起真实请求，也不会创建同步任务。" /><p className="fixed-state">X 未真实同步</p><Toggle label="启用 X 官方 API 同步" checked={x.enabled} onChange={(enabled) => setX({ ...x, enabled })} />{x.enabled && <div className="form-grid"><label className="wide">Bearer Token<input type="password" value={x.token} placeholder={existing?.x?.configured ? '留空表示使用已保存 Token' : ''} onChange={(e) => setX({ ...x, token: e.target.value })} /></label><Toggle label="我已确认 X 开发者政策" checked={x.policyConfirmed} onChange={(policyConfirmed) => setX({ ...x, policyConfirmed })} /></div>}<Footer next={next} nextLabel={x.enabled ? '测试并继续' : '跳过 X'} disabled={x.enabled && ((!x.token && !existing?.x?.configured) || !x.policyConfirmed)} /></>}
      {step === 4 && <><SectionTitle kicker="STEP 05" title="飞书提醒" copy="网页功能不依赖飞书。未启用时不会产生投递、任务或积压。" /><p className="fixed-state">飞书 disabled</p><Toggle label="启用飞书签名机器人" checked={feishu.enabled} onChange={(enabled) => setFeishu({ ...feishu, enabled })} />{feishu.enabled && <div className="form-grid"><label className="wide">Webhook 地址<input value={feishu.webhookUrl} placeholder={existing?.feishu?.configured ? '留空表示使用已保存 Webhook' : ''} onChange={(e) => setFeishu({ ...feishu, webhookUrl: e.target.value })} /></label><label className="wide">签名密钥<input type="password" value={feishu.signingSecret} placeholder={existing?.feishu?.configured ? '留空表示使用已保存密钥' : ''} onChange={(e) => setFeishu({ ...feishu, signingSecret: e.target.value })} /></label></div>}<Footer next={next} nextLabel={feishu.enabled ? '测试并继续' : '跳过飞书'} disabled={feishu.enabled && ((!feishu.webhookUrl || !feishu.signingSecret) && !existing?.feishu?.configured)} /></>}
