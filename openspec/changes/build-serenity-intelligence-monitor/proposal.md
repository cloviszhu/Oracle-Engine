...
- 私有认证：第一阶段仅单家庭管理员、无公开注册、服务端会话、强 Session Secret；具体认证提供方在 design 中基于部署环境选择。
- AI：采用供应商无关适配器，只落地一个模型供应商；默认禁止无证据补全，记录模型版本、成本和失败，预算超限时停止分析并显示待处理，不自动换低质量模型。
- 通知：飞书自定义机器人作为唯一生产渠道；QQ/邮件仅保留适配器边界，不在本 change 实现。
...
- 验收证据：自动化与 Mock 用于可重复行为验证；真实 X、选定 AI 和飞书联调必须单独记录；外部凭据未提供时允许实现继续，但相关 AC 保持未验收状态。
